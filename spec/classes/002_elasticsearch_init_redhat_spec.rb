      ...
      context 'package installation' do
        ...
        context 'via package_url setting' do
          ...
          context 'using http:// schema' do
            ...
            it { should contain_exec('download-package').with(:command => 'wget -O /var/lib/elasticsearch/package.rpm http://www.domain.com/path/to/package.rpm 2> /dev/null', :require => 'File[/var/lib/elasticsearch]') }
            it { should contain_package('elasticsearch').with(:ensure => 'present', :source => '/var/lib/elasticsearch/package.rpm', :provider => 'rpm') }
          end
          ...
          context 'using https:// schema' do
            ...
            it { should contain_exec('create_package_dir').with(:command => 'mkdir -p /var/lib/elasticsearch') }
            it { should contain_file('/var/lib/elasticsearch').with(:purge => false, :force => false, :require => 'Exec[create_package_dir]') }
            it { should contain_exec('download-package').with(:command => 'wget -O /var/lib/elasticsearch/package.rpm https://www.domain.com/path/to/package.rpm 2> /dev/null', :require => 'File[/var/lib/elasticsearch]') }
            it { should contain_package('elasticsearch').with(:ensure => 'present', :source => '/var/lib/elasticsearch/package.rpm', :provider => 'rpm') }
          end
          ...
          context 'using ftp:// schema' do
            ...
            it { should contain_exec('create_package_dir').with(:command => 'mkdir -p /var/lib/elasticsearch') }
            it { should contain_file('/var/lib/elasticsearch').with(:purge => false, :force => false, :require => 'Exec[create_package_dir]') }
            it { should contain_exec('download-package').with(:command => 'wget -O /var/lib/elasticsearch/package.rpm ftp://www.domain.com/path/to/package.rpm 2> /dev/null', :require => 'File[/var/lib/elasticsearch]') }
            it { should contain_package('elasticsearch').with(:ensure => 'present', :source => '/var/lib/elasticsearch/package.rpm', :provider => 'rpm') }
          end
