...
describe 'elasticsearch::service::systemd', :type => 'define' do
  ...
  context "Defaults file" do
	...
	:init_defaults_file => 'puppet:///path/to/initdefaultsfile'
      ...
      it { should contain_file('/etc/sysconfig/elasticsearch-es-01').with(:source => 'puppet:///path/to/initdefaultsfile', :notify => '[Exec[systemd_reload]{:command=>"systemd_reload"}, Service[es-01]{:name=>"es-01"}]', :before => 'Service[es-01]') }
    end
    ...
    context "No restart when 'restart_on_change' is false" do
	  ...
	  :init_defaults_file => 'puppet:///path/to/initdefaultsfile'
        ...
        it { should contain_file('/etc/sysconfig/elasticsearch-es-01').with(:source => 'puppet:///path/to/initdefaultsfile', :notify => 'Exec[systemd_reload]', :before => 'Service[es-01]') }
      end
      ...
      context "Set via hash" do
        ...
        it { should contain_augeas('defaults_es-01').with(:incl => '/etc/sysconfig/elasticsearch-es-01', :changes => "set ES_GROUP 'elasticsearch'\nset ES_HOME '/usr/share/elasticsearch'\nset ES_USER 'elasticsearch'\n", :notify => 'Exec[systemd_reload]', :before => 'Service[es-01]') }
      end
  ...
  context "Init file" do
	...
	:init_template => 'elasticsearch/etc/init.d/elasticsearch.OpenSuSE.erb'
      ...
      it { should contain_file('/usr/lib/systemd/system/elasticsearch-es-01.service').with(:notify => '[Exec[systemd_reload]{:command=>"systemd_reload"}, Service[es-01]{:name=>"es-01"}]', :before => 'Service[es-01]') }
    end
	...
	:init_template => 'elasticsearch/etc/init.d/elasticsearch.OpenSuSE.erb'
      ...
      it { should contain_file('/usr/lib/systemd/system/elasticsearch-es-01.service').with(:notify => 'Exec[systemd_reload]', :before => 'Service[es-01]') }
