...
describe 'elasticsearch::instance', :type => 'define' do
  ...
  context 'config dir' do
    context 'default' do
      ...
      it { should contain_file('/etc/elasticsearch/es-instance/scripts').with(:ensure => 'directory') }
    end
    ...
    context 'set in main class' do
      ...
      it { should contain_file('/usr/share/elasticsearch/scripts') }
      it { should contain_file('/etc/elasticsearch-config/es-instance/scripts').with(:ensure => 'directory') }
    end
  ...
  context 'logging' do
    ...
    describe 'logging ensure' do
      ...
      }.each_pair do |param, config|
        context param do
          describe 'from parent class' do
            let(:pre_condition) do
              %(
                class { 'elasticsearch':
                  #{param}_ensure => 'absent',
                }
              )
            end

            it { should contain_file("/etc/elasticsearch/es-instance/#{config}")
              .with(:ensure => 'absent') }
          end
