...
describe 'elasticsearch::slm_policy', type: 'define' do
  on_supported_os(
    ...
    context "on #{os}" do
      ...
      describe 'policy from source' do
        ...
        it { is_expected.to contain_elasticsearch__slm_policy('foo') }

        it do
          expect(subject).to contain_es_instance_conn_validator('foo-slm_policy-conn-validator').
            that_comes_before('Elasticsearch_slm_policy[foo]')
        end

        it 'passes through parameters' do
          expect(subject).to contain_elasticsearch_slm_policy('foo').with(
            ensure: 'present',
            source: 'puppet:///path/to/foo.json',
            protocol: 'https',
            host: '127.0.0.1',
      ...
      describe 'policy deletion' do
        ...
        it 'removes policy' do
          expect(subject).to contain_elasticsearch_slm_policy('foo').with(ensure: 'absent')
        end
      end
    end
  end
