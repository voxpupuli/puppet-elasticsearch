...
define elasticsearch::template(
  ...
  if ($ensure == 'absent') {
    ...
    file { "${elasticsearch::homedir}/templates_import/elasticsearch-template-${name}.json":
      ensure  => 'absent',
      notify  => Exec[ "delete_template_${name}" ],
      require => File[ "${elasticsearch::homedir}/templates_import" ],
    }
  }
  ...
  if ($ensure == 'present') {
    ...
    exec { "insert_template_${name}":
      command     => "curl ${ssl_args} -sL -w \"%{http_code}\\n\" -XPUT ${es_url} -d @${elasticsearch::homedir}/templates_import/elasticsearch-template-${name}.json -o /dev/null | egrep \"(200|201)\" > /dev/null",
