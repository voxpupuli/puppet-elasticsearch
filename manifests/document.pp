#  This define allows you to insert, update or delete Elasticsearch documents.
#
#  Document content should be defined through either the `content` parameter
#  (when passing a hash or json string) or the `source` parameter (when passing
#  the puppet file URI to a document json file).
#
# @param ensure
#   Controls whether the named document should be present or absent in
#   the cluster.
#
...
#
# @param content
#   Contents of the document. Can be either a puppet hash or a string
#   containing JSON.
#
# @param source
#   Source path for the document file. Can be any value similar to `source`
...
define elasticsearch::document (
  ...
  es_instance_conn_validator { "${name}-document":
    server  => $api_host,
    port    => $api_port,
    timeout => $api_timeout,
  }
  -> elasticsearch_document { $name:
    ensure       => $ensure,
    content      => $_content,
    source       => $source,
